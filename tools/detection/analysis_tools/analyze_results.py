...
from tools.detection.analysis_tools.evaluation.mean_ap import eval_map
from tools.detection.analysis_tools.visualization.image import imshow_gt_det_bboxes
...
from tools.detection.analysis_tools.replace_cfg_vals import replace_cfg_vals


def bbox_map_eval(det_result, annotation, nproc=1):  # 4
...
class ResultVisualizer:
    ...
    def evaluate_and_show_all(self,
                          dataset,
                          results,
                          topk=None,
                          show_dir='work_dir'):
        ...
        if isinstance(results[0], list):
            samples = self.detection_evaluate_all(dataset, results)
        else:
            raise 'The format of result is not supported yet. ' \
                'Current dict for panoptic segmentation and list ' \
                'or tuple for object detection are supported.'

        save_dir = osp.abspath(osp.join(show_dir, 'all'))
        self._save_image_gts_results(dataset, results, samples, save_dir)
    ...
    def detection_evaluate_all(self, dataset, results, eval_fn=None):
        """Evaluation for object detection.
...
def main():
    ...
    if args.topk == 0:
        result_visualizer.evaluate_and_show_all(dataset, outputs, topk=args.topk, show_dir=args.show_dir)
    else:
