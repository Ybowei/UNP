...
class MetaGNCSRoIHead(StandardRoIHead):
    ...
    def __init__(self,
        ...
        self.coco_novel_cls_label = [0, 1, 2, 3, 4, 5, 6, 8, 14, 15, 16, 17, 18, 19, 39, 56, 57, 58, 60, 62]
    ...
    def forward_train(self,
        ...
        for i in range(support_feat.size(0)):
            # if support_gt_labels[i] in self.coco_novel_cls_label:
            #     support_feat_list.append(support_feat[i].unsqueeze(0))
            if support_gt_labels[i] >= 15:
                support_feat_list.append(support_feat[i].unsqueeze(0))
        avg_support_feats = torch.cat(support_feat_list)
        ...
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(
                ...
                query_gt_bboxes, query_gt_labels, support_gt_labels, neg_label_weights)
    ...
    def extract_roi_feat(self, feats: List[Tensor],
                               rois: Tensor) -> Tensor:
