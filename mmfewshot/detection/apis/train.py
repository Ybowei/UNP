...
from mmfewshot.detection.core import (QuerySupportDistEvalHook, QuerySupportEvalHook,
                                      QueryNoSupportDistEvalHook, QueryNoSupportEvalHook)
...
def train_detector(model: nn.Module,
    ...
    if validate:
        ...
        elif 'GNCS' in cfg.model.get('type'):
            # for the GNCS sample methods, the evaluation is the .
            eval_hook = QueryNoSupportDistEvalHook \
                if distributed else QueryNoSupportEvalHook
            runner.register_hook(
    ...
    if 'GNCS' in cfg.model.get('type'):
        backbone_param_dic = {}
        spt_backbone_param_dic = {}
        for name, param in model.module.support_backbone.named_parameters():
            backbone_param_dic[name] = param

        for name, param in model.module.support_backbone.named_parameters():
            spt_backbone_param_dic[name] = param
