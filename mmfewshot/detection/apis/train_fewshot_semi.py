...
def train_detector_semi(model: nn.Module,
                   dataset: Iterable,
                   cfg: ConfigDict,
    ...
    use_clip = hasattr(model, 'CLIPModel') and model.CLIPModel is not None
    # put model on gpus
    if distributed:
        ...
        if use_clip:
            model.module.CLIPModel = MMDistributedDataParallel(
                model.module.CLIPModel.cuda(),
                device_ids=[torch.cuda.current_device()],
                broadcast_buffers=False,
                find_unused_parameters=True)
    else:
        ...
        if use_ema:
            model.module.ema_model = MMDataParallel(model.module.ema_model, device_ids=cfg.gpu_ids)
            # model.ema_model = MMDataParallel(
