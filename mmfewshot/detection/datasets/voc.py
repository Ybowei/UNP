...
class FewShotVOCDataset(BaseFewShotDataset):
    ...
    def __init__(self,
                 ...
                 one_instance_per_image: bool = False,
                 **kwargs) -> None:
        ...
        self.one_instance_per_image = one_instance_per_image
        super().__init__(
            classes=None,
    ...
    def _get_xml_ann_info(self,
        ...
        for obj in root.findall('object'):
            ...
            if difficult:
                continue
            if self.one_instance_per_image:
                break
...
class FewShotVOCDefaultDataset(FewShotVOCDataset):
    ...
    voc_benchmark = {
        f'SPLIT{split}_{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'/data/huang1/PHD_Works/Few_Shot_Object_Detection/mmfewshot/data/few_shot_ann/voc/benchmark_{shot}shot/'
...
@DATASETS.register_module()
class FewShotVOCDataset_Revision(BaseXMLFewShotDataset):
    """VOC dataset for few shot detection.
...
@DATASETS.register_module()
class FewShotVOCDefaultDataset_Revision(FewShotVOCDataset_Revision):
    """Dataset with some pre-defined VOC annotation paths.
            ...
            if ann_root is not None:
                for i in range(len(default_ann_cfg)):
                    default_ann_cfg[i]['ann_file'] = osp.join(
                        ann_root, default_ann_cfg[i]['ann_file'])
            new_ann_cfg += default_ann_cfg
        return super(FewShotVOCDataset_Revision, self).ann_cfg_parser(new_ann_cfg)