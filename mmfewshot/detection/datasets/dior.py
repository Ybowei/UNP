...
DIOR_SPLIT = dict(
    ...
    ALL_CLASSES_SPLIT4=('airplane', 'airport','baseballfield', 'basketballcourt', 'bridge',
    ...
    BASE_CLASSES_SPLIT1=('airplane', 'airport', 'dam', 'Expressway-Service-area', 'Expressway-toll-station',
                      'harbor', 'golffield',  'groundtrackfield', 'overpass', 'stadium',
                      'storagetank', 'tenniscourt', 'trainstation', 'vehicle', 'windmill'),
    BASE_CLASSES_SPLIT2=('baseballfield', 'basketballcourt', 'bridge', 'chimney', 'dam',
                      'Expressway-Service-area', 'golffield', 'overpass', 'ship', 'stadium',
                      'storagetank', 'tenniscourt', 'trainstation', 'vehicle', 'windmill'),
    BASE_CLASSES_SPLIT3=('airplane', 'airport', 'baseballfield', 'basketballcourt', 'bridge',
                     'chimney', 'Expressway-Service-area', 'Expressway-toll-station',  'harbor','groundtrackfield',
                     'overpass', 'ship', 'stadium', 'trainstation', 'windmill'),
    BASE_CLASSES_SPLIT4=('airport', 'basketballcourt', 'bridge', 'chimney', 'dam',
...
class FewShotDIORDataset(BaseFewShotDataset):
    ...
    def load_annotations_xml(
        ...
        for img_name in img_names:
            ...
            elif 'DIOR' in ann_file:
                img_id = img_name
                filename = f'DIOR/JPEGImages/{img_name}.jpg'
            else:
                raise ValueError('Cannot infer dataset year from img_prefix')
            xml_path = osp.join(self.img_prefix, 'DIOR/Annotations', f'{img_id}.xml')
...
class FewShotDIORDefaultDataset(FewShotDIORDataset):
    ...
    dior_benchmark = {
        f'SPLIT{split}_{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'/data/huang1/PHD_Works/Few_Shot_Object_Detection/mmfewshot/data/few_shot_ann/dior/benchmark_{shot}shot/'
                         f'box_{shot}shot_{class_name}_train.txt',
                ann_classes=[class_name])
            for class_name in DIOR_SPLIT[f'ALL_CLASSES_SPLIT{split}']
        ]
        for shot in [10, 20, 30] for split in [1, 2, 3]
    }
