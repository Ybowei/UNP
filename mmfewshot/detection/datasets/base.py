...
@DATASETS.register_module()
class BaseXMLFewShotDataset(BaseFewShotDataset):
    """Base dataset for few shot voc and dior datasets detection.

    The code uses a filtering annotations method that extracts only one instance of an image
    Compared to the BaseFewShotDataset, we only changed the "_filter_annotations" part
    ...
    def _filter_annotations(self, data_infos: List[Dict],
        ...
        for class_name in ann_shot_filter.keys():
            ...
            if len(instance_indices) > num_shots:
                img_idx = {}
                for inst in instance_indices:
                    if inst[0] in img_idx:
                        img_idx[inst[0]].append(inst)
                    else:
                        img_idx[inst[0]] = [inst]

                for cls in img_idx.keys():
                    keep_indices = random.sample(img_idx[cls], 1)
                    keep_instances_indices += keep_indices
